...
public class TileEntityMovingLightHigh extends TileEntity implements ITickableTileEntity
{
	@ObjectHolder("underwaterbiome:moving_light_high")	
    public static TileEntityType<TileEntityMovingLightHigh> TYPE;
	
    public LivingEntity theEntityLiving;
    protected boolean shouldDie = false;
    protected int deathTimer = 2; 
    
	public TileEntityMovingLightHigh() 
	{
		super(TYPE);
	}
...
//		if (world.getBlockState(getPos()).has(BlockInvisible.DISTANCED) && !world.getBlockState(getPos()).get(BlockInvisible.DISTANCED))
		{
			double distanceSquared = getDistanceSq(theEntityLiving.posX, theEntityLiving.posY, theEntityLiving.posZ);
			if (blockAtLocation instanceof BlockInvisible && distanceSquared > 5.0D) 
	        {
	                shouldDie = true;
	                return;
	        }
		}
//		else if(world.getBlockState(getPos()).has(BlockInvisible.DISTANCED) && world.getBlockState(getPos()).get(BlockInvisible.DISTANCED) && EventHandler.rayTraceBlockPos!= null)
//		{
...
//			if (blockAtLocation instanceof BlockInvisible && distanceSquared2 > 3.0D)
//	        {
//                shouldDie = true;
//                return;
//	        }
//		}
        ...
        if (LightingItem.getHoldingLightItem(theEntityLiving).getLight_value()!=3)
        {
            if (blockAtLocation instanceof BlockInvisible)
            {
                shouldDie = true;
...
//    	else 
//    	{
//	    	if (blockAtLocation instanceof BlockInvisible && world.getBlockState(getPos()).get(BlockInvisible.LIGHT) != LightingItem.lightValueToPlace(theEntityLiving))
//	    	{
//	    		shouldDie = true;
//	    	}
//    	}
