...
public class BiomeLightingKelp extends BiomeOceanBase
{
	public BiomeLightingKelp(float depth, float scale,int color, String parent, int weight,Difficulty pollution) 
	{
	      ...
	      this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Biome.createDecoratedFeature(FeatureInit.LIGHTING_KELP, IFeatureConfig.NO_FEATURE_CONFIG, Placement.TOP_SOLID_HEIGHTMAP_NOISE_BIASED, new TopSolidWithNoiseConfig(80, 120.0D, 0.0D, Heightmap.Type.OCEAN_FLOOR_WG)));
...
//	      this.addCarver(GenerationStage.Carving.LIQUID, Biome.createCarver(WorldCarver.UNDERWATER_CAVE, new ProbabilityConfig(0.03F)));
	      ...
	      this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(EntityInit.ENTITY_CONCH_SEA_GRASS, 3, 1, 1));
