...
public class ItemMagicalGun extends ItemAccessoryBase
{

	public ItemMagicalGun(String name, Properties property,ObtainType ob,  boolean needBlueprint, BlueprintType type,
			int... difficulty) 
	{
	...
	{
		return AccessoryEntry.BEFORE_ATTACKING;
	}
	...
	{
		if (target instanceof LivingAttackEvent)
		{
			LivingAttackEvent event = (LivingAttackEvent)target;
			if(player.getHeldItemMainhand().getItem() instanceof IGun)
			{
				DamageSource source = event.getSource().setMagicDamage().setDamageBypassesArmor();
				AccessoryHandler.handleEffect(worldIn, player, new EffectInstance(Effects.WEAKNESS, 60, 0), (LivingEntity) event.getEntityLiving());
				return (T) new LivingAttackEvent(player, source, event.getAmount());
			}
		}
		return target;
	}
